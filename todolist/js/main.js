...
const aplay = document.querySelector('.aplay-todo__item-input')
const btnPlus = document.querySelector('.btn--plus')
const todolist = document.querySelector('.box__item ul')
const donelist = document.querySelector('.done-box__item ul')
const clear = document.querySelector('.btn--clear-todo')

function createItem(list,item) {
    // console.log('aa')
    //tạo nội dung với 1 thẻ li và 2 nút button
    const todoItemLi = document.createElement('li')
    const todoItemCheck = document.createElement('button')
    ...
    //
    const todoBoxList = document.createElement('div')
...
function action (e) {
    ...
    // ở đây mk bắt xem n click vào nút
    ...
    if(e.target.classList[0] === 'btn--check') {
        ...
        // xóa bên todo và n chuyển sang done-list
        item.style.opacity = '0'
        item.style.transform = 'translateX(100%)'
        item.addEventListener('transitionend', function() {
            donelist.appendChild(item)
            // sau khi hết hiệu ứng mk add item qua bên done-list
