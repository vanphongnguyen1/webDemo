    ...
    function listStudents (list,item) {
        ...
        if(!name) {
            document.querySelector('.error-name').innerHTML='<font color="f00"> Vui lòng điền: Họ và Tên</font>'
        } else {
            document.querySelector('.error-name').innerHTML=''
        }
        if(!email) {
            document.querySelector('.error-email').innerHTML='<font color="f00"> Vui lòng điền: Email</font>'
        } else {
            document.querySelector('.error-email').innerHTML=''
        }
        if(!phone) {
            document.querySelector('.error-phone').innerHTML='<font color="f00"> Vui lòng điền: Số điện thoại</font>'
        } else {
            document.querySelector('.error-phone').innerHTML=''
        }
        if(!address) {
            document.querySelector('.error-address').innerHTML='<font color="f00"> Vui lòng điền: Address</font>'
        } else {
            document.querySelector('.error-address').innerHTML=''
        }
        if(!date) {
            document.querySelector('.error-date').innerHTML='<font color="f00"> Vui lòng chọn: Ngày/Tháng/Năm</font>'
        } else {
            document.querySelector('.error-date').innerHTML=''
        }
        if(!gender) {
            document.querySelector('.error-gender').innerHTML='<font color="f00"> Vui lòng chọn: Giới tính</font>'
        } else {
